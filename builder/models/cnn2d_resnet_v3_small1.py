...
class CNN2D_RESNET_V3_SMALL1(nn.Module):
        def __init__(self, args, device):
                super(CNN2D_RESNET_V3_SMALL1, self).__init__()      
                self.args = args
                self.dropout = 0.1
                ...
                self.features = nn.Sequential(
                        conv2d_bn(1,  self.in_planes, (1,51), (1,1), (0,25)), 
                        nn.MaxPool2d(kernel_size=(1,4), stride=(1,4)),
                )

                block = BasicBlock
                self.layer1 = self._make_layer(block, 16, 2, stride=2)
